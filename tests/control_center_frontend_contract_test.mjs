...
for (const marker of ['control-center/app.js','import(path)','2026-07-16-inbox-writeback-v4']) if (!loader.includes(marker)) errors.push(`module loader missing: ${marker}`);
if (!html.includes('control-center.js?v=2026-07-16-inbox-writeback-v4')) errors.push('top-level control-center script cache key is stale');
...
for (const marker of ['google_sheets_direct_batch','values:batchUpdate','BE_CC_INBOX_DIRECT_HTTP_TIMEOUT_SECONDS','BE_CC_INBOX_DIRECT_VERIFY_ATTEMPTS','be_cc_verify_inbox_writeback_direct']) if (!directInboxWriteback.includes(marker)) errors.push(`bounded direct inbox writeback missing: ${marker}`);
