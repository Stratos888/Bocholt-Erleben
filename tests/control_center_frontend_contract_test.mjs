...
for (const marker of ['/api/growth-backlog/list.php','Informations- und Reihenfolgeansicht','alle sichtbar','Offen · empfohlene Reihenfolge','Abgeschlossen','<details class="cc-labor-row cc-roadmap-row','<summary>','cc-labor-detail']) if (!app.includes(marker)) errors.push(`roadmap frontend contract missing: ${marker}`);
...
for (const forbidden of ['data-backlog-action','cc-new-backlog','submitBacklog(','handleBacklog(','backlogCases()','<article class="cc-roadmap-item']) if (app.includes(forbidden)) errors.push(`roadmap must not behave like a work queue or permanently expanded card list: ${forbidden}`);
