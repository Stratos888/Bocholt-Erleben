...
cd "$ROOT"
command -v docker >/dev/null 2>&1 || { printf '%s\n' 'Docker is required for the Startpartner Gate-4 database contract.' >&2; exit 2; }
command -v php >/dev/null 2>&1 || { printf '%s\n' 'PHP is required for the Startpartner Gate-4 database contract.' >&2; exit 2; }
if ! php -r 'exit(extension_loaded("pdo_mysql") ? 0 : 1);'; then
  if command -v sudo >/dev/null 2>&1; then sudo apt-get update -qq; sudo apt-get install -y -qq php-mysql; else printf '%s\n' 'pdo_mysql is required.' >&2; exit 2; fi
fi
...
expected=[f'{n:03d}' for n in range(1,14)]
...
run_engine(){
  ...
  local port; port="$(docker inspect -f '{{(index (index .NetworkSettings.Ports "3306/tcp") 0).HostPort}}' "$container")"
  for contract in tests/startpartner_gate4_schema_contract_test.php tests/startpartner_gate4_mysql_contract_test.php tests/startpartner_active_pilot_lifecycle_mysql_contract_test.php tests/startpartner_gate4_v2_journey_contract_test.php; do
    printf '[%s] Running %s\n' "$engine" "$contract"
    STARTPARTNER_TEST_DSN="mysql:host=127.0.0.1;port=${port};dbname=be_contract;charset=utf8mb4" STARTPARTNER_TEST_USER=root STARTPARTNER_TEST_PASSWORD=contract-root php "$contract"
  done
  docker rm -f "$container" >/dev/null
}
run_engine mysql8 mysql:8.0 mysql
run_engine mariadb114 mariadb:11.4 mariadb
printf '%s\n' '=== Startpartner Gate-4 MySQL 8 and MariaDB 11.4 Contracts: OK ==='
