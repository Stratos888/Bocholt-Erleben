...
$assert(str_contains($deploySmoke, 'check_removed_gate3_temporary_endpoints'), 'Deploy-Smoke muss den Rückbau der Gate-3-Evidence-Endpunkte prüfen.');
foreach ([
    '/api/startpartner/_gate3_staging_migration_231.php',
    '/api/startpartner/_gate3_staging_lifecycle_231.php',
    'HTTP 404',
] as $marker) {
    $assert(str_contains($deploySmoke, $marker), "Gate-3-404-Nachweis fehlt: {$marker}");
}
foreach ([
    ...
    $assert(!str_contains($deploySmoke, $removedEvidenceToken), "Generischer Deploy-Smoke enthält noch Gate-3-Evidence-Wiring: {$removedEvidenceToken}");
}
foreach ([
    'gate2-staging-status-199.php',
    'gate2-staging-lifecycle-199.php',
