...
$expect=static function(callable $callback,string $message)use(&$failures):void{try{$callback();$failures[]=$message;}catch(InvalidArgumentException $expected){}};
...
$assert($window['planned_end_date']==='2027-04-25','Six calendar months must be calculated exactly.');
...
$expect(static fn()=>be_startpartner_gate4_validate_local_date('2026-02-30'),'Invalid local dates must fail.');
$expect(static fn()=>be_startpartner_gate4_content_type('place'),'Unsupported content types must fail.');

if($failures){fwrite(STDERR,"=== Startpartner Gate-4 Domain Contract: FAILED ===\n".implode("\n",array_map(static fn($v)=>'- '.$v,$failures))."\n");exit(1);}echo "=== Startpartner Gate-4 Domain Contract: OK ===\n";
