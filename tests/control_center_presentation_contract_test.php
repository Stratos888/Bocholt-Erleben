...
$buildKey='2026-07-24-mobile-exception-review-v1';
$cssGovernanceKey='2026-06-22-css-governance-v1';
assert_contains_text($html,'control-center.js?v=' . $buildKey,'Top-Level-Cachekey muss den mobilen Ausnahmeprüfungs-Build laden');
assert_contains_text($html,'style.css?v=' . $cssGovernanceKey,'Top-Level-CSS muss den stabilen Governance-Key behalten');
assert_contains_text($loader,'control-center/app.js?v=' . $buildKey,'Loader muss den aktuellen App-Build laden');
assert_contains_text($app,'review.js?v=' . $buildKey,'App muss das aktuelle Review-Modul laden');
assert_contains_text($reviewModule,'review-render.js?v=' . $buildKey,'Review-Modul muss den aktuellen Renderer laden');
...
assert_contains_text($renderer,'matched_title','bestehender Treffer muss mit vorhandenem Titel vergleichbar sein');
assert_contains_text($renderer,'matched_date','bestehender Treffer muss mit vorhandenem Datum vergleichbar sein');
assert_contains_text($renderer,'matched_location','bestehender Treffer muss mit vorhandenem Ort vergleichbar sein');
assert_contains_text($renderer,'Entscheidung auswählen','mehrere mobile Resultate brauchen eine einzelne Entscheidungsebene');
assert_contains_text($renderer,'trueWaiting','echte Wartezustände müssen von Freigaben getrennt bleiben');
assert_contains_text($renderer,'aria-expanded','mobile Disclosure-Zustände müssen ihren Öffnungszustand zugänglich melden');
assert_true(!str_contains($renderer,'matched_time'),'nicht gelieferte Bestandsuhrzeit darf nicht erfunden werden');
