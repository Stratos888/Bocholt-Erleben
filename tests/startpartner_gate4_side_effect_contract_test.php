...
$files=[
    ...
    'api/submissions/_approval_domain.php',
...
$forbidden=['api/stripe','stripe_checkout','stripe_subscription_id =','be_send_mail(','request-magic-link.php','Formspree','Events_Staging','publication_entitlements SET','publication_consumptions ('];
foreach($forbidden as $token){if(str_contains($combined,$token)){fwrite(STDERR,"Forbidden Gate-4 side effect token: {$token}\n");exit(1);}}
$required=['startpartner_pilot_usage','pending_activation','Europe/Berlin','converted_to_pilot','capacity_delta','payment_kind=\'pilot\'','intake_origin=\'startpartner_pilot\''];
