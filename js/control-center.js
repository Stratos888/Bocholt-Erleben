(() => {
  ...
  async function openContentEditor(id) {
    ...
    const data = await api(`/api/control-center/content.php?type=${encodeURIComponent(type)}&id=${encodeURIComponent(id)}`);
    const item = data.item;
    if (type === 'activities' && item.editable === false) {
      openDialog(`<h2>${escapeHtml(item.title)}</h2><p>${escapeHtml(item.edit_notice)}</p><p>Die öffentliche Vorschau kann geöffnet werden. Der Editor wird erst nach vollständigem Repo-, Deploy- und Veröffentlichungsnachweis freigeschaltet.</p><a class="cc-button cc-button--secondary" href="${escapeHtml(item.public_url)}" target="_blank" rel="noopener">Vorschau öffnen</a>`);
      return;
    }
