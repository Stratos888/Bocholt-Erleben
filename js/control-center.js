(() => {
  ...
  function reviewCases() { return activeCases().filter(i => i.type === 'intake' && (state.reviewGroup === 'all' || i.queue_group === state.reviewGroup)); }
  ...
  function renderOverview() {
    const cases = activeCases(), urgent = cases.filter(i => i.bucket === 'now'), reviews = cases.filter(i => i.type === 'intake'), tasks = cases.filter(i => i.type === 'task'), waiting = tasks.filter(i => ['waiting','snoozed'].includes(i.state)), backlog = backlogCases().filter(i => !['done','rejected','parked'].includes(i.state));
    ...
    els.view.innerHTML = `<section class="cc-overview-grid">
      ...
      <article class="cc-summary-card cc-summary-card--quiet"><div><span class="cc-summary-label">Entwicklung</span><h2>${escapeHtml(state.development?.summary?.label || 'Daten werden geladen')}</h2><p>${state.development ? `${state.development.content_quality?.coverage_percent || 0} % Eventbasis vollständig · ${state.development.product?.open_repo_workpacks || 0} offene Workpacks` : 'Projektwirkung und SEO-Signale werden zusammengeführt.'}</p></div><button class="cc-button cc-button--secondary" data-go-view="development">Entwicklung öffnen</button></article>
    </section>`;
    bindViewLinks();
  }
  ...
  function renderReview() {
    const items = reviewCases(); if (state.reviewIndex >= items.length) state.reviewIndex = Math.max(0, items.length - 1); const active = items[state.reviewIndex];
  ...
  function laborRow(item) {
    ...
    return `<details class="cc-labor-row" data-case-id="${escapeHtml(item.id)}"><summary><span><b>${escapeHtml(item.title)}</b><small>${escapeHtml(kind)}${item.due_at ? ` · ${escapeHtml(formatDate(item.due_at))}` : ''}</small></span><span class="cc-labor-summary">${escapeHtml(summary)}</span></summary><div class="cc-labor-detail">${item.reason ? `<p>${escapeHtml(item.reason)}</p>` : ''}${item.next_action ? `<p><strong>Nächster Schritt:</strong> ${escapeHtml(item.next_action)}</p>` : ''}${item.waiting_for ? `<p><strong>Wartet auf:</strong> ${escapeHtml(item.waiting_for)}</p>` : ''}<div class="cc-actions">${item.primary_action ? `<button class="cc-button cc-button--primary" data-labor-action="${escapeHtml(item.primary_action.key)}">${escapeHtml(item.primary_action.label)}</button>` : ''}${(item.secondary_actions || []).map(a => `<button class="cc-button ${a.destructive ? 'cc-button--danger' : 'cc-button--secondary'}" data-labor-action="${escapeHtml(a.key)}">${escapeHtml(a.label)}</button>`).join('')}<button class="cc-button cc-button--ghost" data-labor-action="details">Details</button></div></div></details>`;
  }
  ...
  async function loadManagement(type) { if (state.managementLoaded[type]) return; const result = await api(`/api/control-center/content.php?type=${encodeURIComponent(type)}`); state.managementItems[type] = result.items || []; state.managementLoaded[type] = true; }
  function managementRow(item, type) { const detail = [item.date, item.location, item.city, item.category].filter(Boolean).join(' · '); return `<article class="cc-manage-row"><div><span class="cc-kicker">${type === 'events' ? 'Veranstaltung' : 'Aktivität'} · ${escapeHtml(item.status || 'veröffentlicht')}</span><h3>${escapeHtml(item.title)}</h3>${detail ? `<p>${escapeHtml(detail)}</p>` : ''}</div><div class="cc-actions"><a class="cc-button cc-button--secondary" href="${escapeHtml(item.public_url)}" target="_blank" rel="noopener">Vorschau</a><button class="cc-button cc-button--primary" data-manage-edit="${escapeHtml(item.id)}">Bearbeiten</button></div></article>`; }
  async function renderManage() {
    els.view.innerHTML = '<div class="cc-empty">Inhalte werden geladen …</div>';
    try { await loadManagement(state.managementType); const source = state.managementItems[state.managementType] || [], q = state.managementQuery.toLowerCase(), items = source.filter(i => !q || clean(i.title).toLowerCase().includes(q)).slice(0,100);
      els.view.innerHTML = `<div class="cc-toolbar cc-toolbar--manage"><div class="cc-segment"><button class="${state.managementType === 'events' ? 'is-active' : ''}" data-manage-type="events">Veranstaltungen</button><button class="${state.managementType === 'activities' ? 'is-active' : ''}" data-manage-type="activities">Aktivitäten</button></div><label class="cc-search"><span class="sr-only">Suchen</span><input id="cc-manage-search" type="search" value="${escapeHtml(state.managementQuery)}" placeholder="Suchen …"></label></div><div id="cc-manage-results" class="cc-manage-list">${items.map(i => managementRow(i,state.managementType)).join('') || '<div class="cc-empty cc-empty--large">Keine passenden Inhalte gefunden.</div>'}</div>`; bindManage();
    } catch (error) { els.view.innerHTML = `<div class="cc-empty cc-empty--large">${escapeHtml(error.message)}</div>`; }
  }
  ...
  async function renderDevelopment() {
    if (!state.development) { els.view.innerHTML = '<div class="cc-empty">Entwicklungsdaten werden geladen …</div>'; try { state.development = await api('/api/control-center/development.php'); } catch (e) { els.view.innerHTML = `<div class="cc-empty cc-empty--large">${escapeHtml(e.message)}</div>`; return; } }
    const d = state.development, q = d.content_quality || {}, a = d.automation || {}, s = d.seo || {}, p = d.product || {};
    els.view.innerHTML = `<section class="cc-development-head"><span class="cc-kicker">Gesamtprojekt</span><h2>${escapeHtml(d.summary?.label || 'Entwicklungsstand')}</h2><p>Datenstand ${escapeHtml(formatDate(d.generated_at))}</p></section><section class="cc-metric-grid">
      ${metricCard('Content-Qualität', `${q.coverage_percent || 0} %`, `${q.complete_events || 0} von ${q.events_total || 0} Events mit Beschreibung, Quelle und Datum`, q.open_quality_reviews > 0 ? 'attention' : 'good')}
      ${metricCard('Automatisierung', `${a.completed_cases || 0} erledigt`, `${a.open_reviews || 0} offene Prüfungen · ${a.blocked_tasks || 0} blockiert`, a.blocked_tasks > 0 ? 'attention' : 'good')}
      ${metricCard('SEO-Onpage', `${s.onpage_event_coverage_percent || 0} %`, `${s.missing_descriptions || 0} fehlende Beschreibungen · ${s.missing_sources || 0} fehlende Quellen`, s.missing_descriptions > 0 || s.missing_sources > 0 ? 'attention' : 'good')}
      ${metricCard('Produktentwicklung', `${p.open_repo_workpacks || 0} offen`, 'Kuratierte, verbindliche Repo-Workpacks', '')}
    </section><section class="cc-section"><h2>SEO-Datenlage</h2><p>${escapeHtml(s.search_console_message || '')}</p></section><section class="cc-section"><h2>Aktuelle Workpacks</h2><div class="cc-link-list">${(p.workpacks || []).map(w => `<div class="cc-link-card"><strong>${escapeHtml(w.title || w.id)}</strong><span>${escapeHtml(w.next_action || w.description || '')}</span></div>`).join('') || '<div class="cc-empty">Keine offenen Workpacks.</div>'}</div></section>`;
  }
  ...
  function updateBadges() { const reviews = activeCases().filter(i => i.type === 'intake').length, labor = activeCases().filter(i => i.type === 'task').length; [[els.badgeReview,reviews],[els.badgeLabor,labor]].forEach(([e,n]) => { e.hidden = n === 0; e.textContent = n > 99 ? '99+' : String(n); }); }
  ...
  function openNewWorkMenu() { openDialog(`<h2>Neuen Eintrag anlegen</h2><div class="cc-link-list"><button class="cc-link-card" data-new-kind="task"><strong>Aufgabe</strong><span>Konkrete beschlossene Arbeit</span></button><button class="cc-link-card" data-new-kind="backlog"><strong>Backlogpunkt</strong><span>Geplante, noch nicht gestartete Arbeit</span></button><button class="cc-link-card" data-new-kind="idea"><strong>Idee</strong><span>Noch ungeprüfter Gedanke</span></button></div>`); document.querySelectorAll('[data-new-kind]').forEach(b => b.addEventListener('click', () => { const kind = b.dataset.newKind; closeDialog(); kind === 'backlog' ? openBacklogDialog() : openCreateDialog(kind); })); }
  ...
  async function openContentEditor(id) { const type = state.managementType; const data = await api(`/api/control-center/content.php?type=${encodeURIComponent(type)}&id=${encodeURIComponent(id)}`); const item = data.item; if (type === 'activities' && item.editable === false) { openDialog(`<h2>${escapeHtml(item.title)}</h2><p>${escapeHtml(item.edit_notice)}</p><p>Die öffentliche Vorschau kann geöffnet werden; eine dauerhafte Bearbeitung folgt erst mit dem abgesicherten Repo-Writeback.</p><a class="cc-button cc-button--secondary" href="${escapeHtml(item.public_url)}" target="_blank" rel="noopener">Vorschau öffnen</a>`); return; }
    const field = (id,label,value,type='text') => `<label class="cc-field"><span>${label}</span><input id="${id}" type="${type}" value="${escapeHtml(value || '')}"></label>`;
    openDialog(`<h2>Veranstaltung bearbeiten</h2><div class="cc-stack">${field('ce-title','Titel',item.title)}${field('ce-date','Startdatum',item.date,'date')}${field('ce-end','Enddatum',item.end_date,'date')}${field('ce-time','Uhrzeit',item.time)}${field('ce-location','Ort',item.location)}${field('ce-city','Stadt',item.city)}${field('ce-category','Kategorie',item.category)}${field('ce-source','Offizielle Quelle',item.source_url,'url')}${field('ce-ticket','Ticketlink',item.ticket_url,'url')}<label class="cc-field"><span>Beschreibung</span><textarea id="ce-description">${escapeHtml(item.description || '')}</textarea></label><button type="button" class="cc-button cc-button--primary" id="ce-save">Speichern und veröffentlichen</button></div>`);
    document.querySelector('#ce-save').addEventListener('click', async () => { const v = sel => clean(document.querySelector(sel).value); const updates = { title:v('#ce-title'), date:v('#ce-date'), endDate:v('#ce-end'), end_date:v('#ce-end'), time:v('#ce-time'), location:v('#ce-location'), city:v('#ce-city'), kategorie:v('#ce-category'), source_url:v('#ce-source'), ticket_url:v('#ce-ticket'), description:v('#ce-description') }; closeDialog(); setStatus('Änderung wird gespeichert …'); try { const result = await api('/api/control-center/content.php', { method:'POST', body:JSON.stringify({ type:'events', id:item.id, updates, deploy:true }) }); state.managementLoaded.events = false; await renderManage(); setStatus(result.message || 'Änderung gespeichert und Deployment gestartet.'); } catch (e) { setStatus(e.message); } });
  }
  ...
  function openSystemDialog() { const sync = state.overview?.sync || {}; closeDialog(); openDialog(`<h2>Systemstatus</h2><p>${escapeHtml(state.overview?.system?.message || 'Keine bekannte Störung mit Auswirkung.')}</p><ul class="cc-system-list"><li><strong>Prüffälle</strong><span>${activeCases().filter(i => i.type === 'intake').length} offen</span></li><li><strong>Arbeit</strong><span>${activeCases().filter(i => i.type === 'task').length} aktiv</span></li><li><strong>Blockiert</strong><span>${activeCases().filter(i => i.state === 'blocked').length}</span></li></ul><details class="cc-disclosure"><summary>Technische Details</summary><div>${escapeHtml(JSON.stringify(sync,null,2))}</div></details>`); }
  ...
  els.refresh.addEventListener('click', () => { state.managementLoaded = { events:false, activities:false }; state.development = null; load(); });
