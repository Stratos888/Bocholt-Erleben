...
function renderOverview() {
  ...
  const backlogText = state.backlog?.status === 'error'
    ? 'Kanonischer Backlog konnte nicht geladen werden'
...
export async function load(options = {}) {
  ...
  try {
    ...
    state.backlog = backlogResult.ok
      ...
      : { status:'error', label:'Growth-Backlog', items:[], counts:{ total:0, open:0, completed:0 }, message:backlogResult.error?.message || 'Backlog konnte nicht geladen werden.' };
    updateBadges();
    render();
    setStatus('');
