...
function plannedEndDue(gate4={}){const end=clean(gate4.pilot?.planned_end_date).slice(0,10);return ['active','paused'].includes(clean(gate4.phase))&&/^\d{4}-\d{2}-\d{2}$/.test(end)&&berlinToday()>=end;}

function controlNextAction(gate4={}) {
  const phase=clean(gate4.phase);
  if(['ended_without_conversion','terminated','converted'].includes(phase))return {code:'none',label:'Pilot abgeschlossen'};
  if(phase==='closing')return {code:'end_without_conversion',label:'Pilot geordnet abschließen',action:'end_without_conversion'};
  if(plannedEndDue(gate4))return {code:'closeout_required',label:'Pilotende jetzt entscheiden',action:'start_closeout'};
  const dueCheckpoint=asArray(gate4.lifecycle?.checkpoints).find(row=>row?.status==='due');
  if(dueCheckpoint)return {code:'checkpoint_due',label:`${checkpointLabels[dueCheckpoint.checkpoint_key]||'Pilot-Checkpoint'} abschließen`,action:'complete_checkpoint',checkpoint_key:dueCheckpoint.checkpoint_key};
  const measurement=gate4.measurement_runtime||{};
  if(['query_or_attribution_problem','technical_not_ready'].includes(measurement.status))return {code:'measurement_problem',label:'Technische Erfolgsmessung prüfen',action:'measurement'};
  const distribution=gate4.distribution_runtime||{};
  if(['due','blocked'].includes(distribution.status)&&distribution.commitment?.id)return {code:distribution.status==='blocked'?'distribution_blocked':'distribution_due',label:'Reichweitenbeitrag klären',action:'set_distribution_fulfillment',distribution_id:distribution.commitment.id};
  if(phase==='active'){
    const draft=asArray(gate4.content_links).find(row=>row.status==='draft');
    if(draft)return {code:'content_review',label:'Nächsten Pilotinhalt redaktionell prüfen',action:'mark_content_ready',content_link_id:draft.id};
    const ready=asArray(gate4.content_links).find(row=>row.status==='editorial_ready');
    if(ready)return {code:'content_approval',label:'Vorbereiteten Pilotinhalt freigeben',action:'approve_content',content_link_id:ready.id};
  }
  if(phase==='paused')return {code:'paused',label:'Pilot fortsetzen',action:'resume'};
...
function protectedControls(gate4={}) {
  ...
  return `<details class="cc-disclosure"><summary>Pilot steuern</summary><p class="cc-muted">Diese Aktionen ändern den Pilotstatus. Sie lösen weder Zahlungen noch eine automatische Veröffentlichung oder Löschung aus.</p><div class="cc-actions">${buttons.join('')}</div></details>`;
}
...
async function contentApproveDialog(item,contentLinkId,reload){const detail=await latest(item),data=detail.startpartner_candidate,row=asArray(data.gate4?.content_links).find(entry=>String(entry.id)===contentLinkId);openDialog(`<h2>Pilotinhalt freigeben</h2><p>${escapeHtml(row?.title||'Ausgewählter Inhalt')}</p><div class="cc-notice cc-notice--attention"><strong>Verbrauch wird jetzt geprüft</strong><span>Bei der Freigabe werden Laufzeit und das Event- bzw. Aktivitätslimit serverseitig erneut geprüft. Nur bei Erfolg entsteht genau eine Pilotnutzung.</span></div><div id="cc-dialog-message"></div>${confirmButton('Inhalt freigeben')}`,'cc-dialog--wide');document.querySelector('#gate4-confirm')?.addEventListener('click',async event=>{event.currentTarget.disabled=true;const result=await mutate('/api/startpartner/lifecycle.php',data,{action:'approve_content',content_link_id:contentLinkId},reload,'Pilotinhalt freigegeben und einmalig dem Pilotverbrauch zugeordnet.');if(!result)event.currentTarget.disabled=false;});}

async function lifecycleDialog(item,transition,reload){const detail=await latest(item),data=detail.startpartner_candidate,config={pause:['Pilot pausieren','Während der Pause bleiben bisherige Inhalte und Nutzungen unverändert. Neue Pilotinhalte sind gesperrt.','Pilot pausieren','secondary'],resume:['Pilot fortsetzen','Der Pilot wird nur fortgesetzt, wenn seine wirksame Laufzeit noch besteht.','Pilot fortsetzen','primary'],start_closeout:['Pilotabschluss einleiten','Der Abschlusszustand ist in diesem Workpack nicht rückgängig zu machen. Neue Pilotinhalte werden gesperrt.','Abschluss einleiten','primary'],end_without_conversion:['Pilot geordnet beenden','Der Pilot endet ohne automatische kostenpflichtige Fortführung. Offene, noch nicht verbrauchte Pilotzuordnungen werden zurückgezogen.','Pilot beenden','primary'],terminate:['Pilot abbrechen','Der Pilot wird kontrolliert abgebrochen. Historische Nutzungen bleiben erhalten; es wird nichts automatisch gelöscht oder kostenpflichtig fortgeführt.','Pilot abbrechen','danger']}[transition];if(!config)return;openDialog(`<h2>${escapeHtml(config[0])}</h2><p>${escapeHtml(config[1])}</p><div id="cc-dialog-message"></div>${confirmButton(config[2],config[3])}`,'cc-dialog--wide');document.querySelector('#gate4-confirm')?.addEventListener('click',async event=>{event.currentTarget.disabled=true;const result=await mutate('/api/startpartner/lifecycle.php',data,{action:transition},reload,`${config[0]}: aktueller Stand neu geladen.`);if(!result)event.currentTarget.disabled=false;});}
