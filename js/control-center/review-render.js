...
import { renderStartpartnerReview } from './startpartner-review.js?v=2026-08-27-startpartner-premium-finish-v1';
import { startpartnerAiReviewStatuses, aiReviewStatusLabel } from './startpartner-ai-review.js?v=2026-08-21-ai-assisted-review-v1';
...
function startpartnerData(item) {
  return item?.startpartner_candidate || {
    status:item?.decision_context?.candidate_status||'',
    gate4: item?.decision_context?.gate4 || null,
  };
}
function caseStatus(item) {
  const data = startpartnerData(item);
  if (item?.case_kind === 'startpartner_candidate' && data?.gate4?.pilot) return gate4PhaseLabel(data.gate4.phase);
  if (item?.case_kind === 'startpartner_candidate' && startpartnerAiReviewStatuses.has(String(data?.status||''))) return aiReviewStatusLabel(data.status);
...
function renderTask(task) {
  const evidence=task.evidence||{}; const actions=asArray(task.actions); const asset=evidence.asset||evidence.fallback||null; const evidenceContent=taskEvidence(evidence,asset);
  return `<article class="cc-review-task cc-review-task--${escapeHtml(task.state||'open')}" data-review-task="${escapeHtml(task.task_id)}" tabindex="-1"><header><div><span class="cc-kicker">${escapeHtml(taskStateLabels[task.state]||task.state||'Offen')}</span><h3>${escapeHtml(task.title||'Prüfpunkt')}</h3></div>${evidenceBadge(evidence)}</header><p>${escapeHtml(task.message||'')}</p>${evidenceContent?`<div class="cc-review-task-evidence--desktop">${evidenceContent}</div><details class="cc-review-task-evidence--mobile"><summary>Evidence und Begründung</summary><div>${evidenceContent}</div></details>`:''}${actions.length?`${taskActionButtons(task,'cc-review-task-actions--desktop')}<details class="cc-review-task-options"><summary>Weitere Optionen</summary>${taskActionButtons(task,'cc-review-task-actions--mobile')}</details>`:''}</article>`;
}
...
function verifiedFactsContent(review) {
  ...
  const date=[facts.date?formatDate(facts.date):'',facts.end_date?`bis ${formatDate(facts.end_date)}`:''].filter(Boolean).join(' '); const time=facts.time_details||facts.time||timeLabels[facts.time_status]||''; const asset=visual.asset; const visualKey=visual.key_label||visual.key; const visualMotif=visual.motif_label||visual.motif; const visualAsset=visual.asset_label||(asset?'Freigegebenes Eventbild':'');
  return `<dl class="cc-fact-grid">${verifiedFact('Termin',date,evidence.date)}${verifiedFact('Uhrzeit',time,evidence.time_details||evidence.time||evidence.time_status)}${verifiedFact('Ort',[facts.location,facts.city].filter(Boolean).join(' · '),evidence.location)}${verifiedFact('Adresse',facts.address,evidence.address)}${verifiedFact('Kategorie',classification.category,evidence.category)}${verifiedFact('Prüfquelle',source.name||source.url,evidence.source_url,source.url)}${verifiedFact('Eventseite',source.event_url,evidence.event_url,source.event_url)}${verifiedFact('Ticket / Anmeldung',source.ticket_url,evidence.ticket_url,source.ticket_url)}${verifiedFact('Bildbereich',visualKey,evidence.visual_key)}${verifiedFact('Motiv',visualMotif,evidence.visual_motif)}${verifiedFact('Gebundenes Bild',visualAsset,evidence.visual_asset_id)}</dl>${assetPreview(asset)}<section class="cc-detail-section"><h3>Finale Beschreibung</h3><p class="cc-description-copy">${escapeHtml(review.description?.final||'Keine Beschreibung vorhanden.')}</p></section>`;
}
...
function comparisonFact(label,value,formatter=value=>value) { const cleaned=clean(value); if(!cleaned)return ''; return `<div><dt>${escapeHtml(label)}</dt><dd>${escapeHtml(formatter(cleaned))}</dd></div>`; }
function formatMatchScore(value) { const text=clean(value); if(!text)return ''; if(text.includes('%'))return text; const numeric=Number(text.replace(',','.')); if(!Number.isFinite(numeric))return text; const percent=numeric<=1?numeric*100:numeric; return `${Number.isInteger(percent)?percent:percent.toFixed(1).replace('.',',')} %`; }
function renderDuplicateComparison(review,tasks) {
  const task=duplicateTask(tasks); if(!task)return ''; const evidence=task.evidence||{}, facts=review.facts||{}; const candidateLocation=[facts.location,facts.city].filter(Boolean).join(' · '); const score=formatMatchScore(evidence.score||review.dedupe?.score); const sourceUrl=clean(evidence.source_url||review.dedupe?.url);
  return `<section class="cc-duplicate-comparison" aria-label="Kandidat und bestehendes Event vergleichen"><header><div><span class="cc-kicker">Treffervergleich</span><strong>Kandidat und Bestand</strong></div>${score?`<span class="cc-match-score">Match ${escapeHtml(score)}</span>`:''}</header><div class="cc-duplicate-comparison__columns"><section><span class="cc-duplicate-comparison__label">Kandidat</span><dl>${comparisonFact('Titel',facts.title)}${comparisonFact('Datum',facts.date,formatDate)}${comparisonFact('Ort',candidateLocation)}</dl></section><section><span class="cc-duplicate-comparison__label">Bestehendes Event</span><dl>${comparisonFact('Titel',evidence.matched_title)}${comparisonFact('Datum',evidence.matched_date,formatDate)}${comparisonFact('Ort',evidence.matched_location)}</dl>${sourceUrl?`<a href="${escapeHtml(sourceUrl)}" target="_blank" rel="noopener">Bestehendes Event öffnen</a>`:''}</section></div></section>`;
}
function trueWaiting(item,tasks) { if(clean(item?.waiting_for))return true; return tasks.length>0&&tasks.every(task=>task.state==='waiting_external'); }
function renderMobileDecision(review,item,tasks) {
  ...
  const task=tasks.find(entry=>entry.state!=='waiting_external'&&asArray(entry.actions).length)||tasks.find(entry=>asArray(entry.actions).length); if(!task)return '<section class="cc-mobile-decision cc-mobile-decision--waiting"><strong>Keine unmittelbare Aktion</strong><span>Der Fall bleibt sichtbar, bis eine belastbare Entscheidung möglich ist.</span></section>';
  const actions=asArray(task.actions); if(actions.length===1)return `<section class="cc-mobile-decision"><span>${escapeHtml(task.title||'Nächsten Prüfpunkt entscheiden')}</span>${taskActionButtons(task,'cc-review-task-actions--focus')}</section>`;
...
export function renderEventReview(review,item={}) { if (!review) return ''; const tasks=asArray(review.review_tasks); return `<section class="cc-review-data"><div class="cc-mobile-priority">${renderDuplicateComparison(review,tasks)}${renderMobileDecision(review,item,tasks)}</div>${renderGate(review)}${tasks.length?`<section class="cc-review-task-list" aria-label="Offene Prüfaufgaben">${tasks.map(renderTask).join('')}</section>`:''}${renderVerifiedFacts(review)}</section>`; }
function generic(item) { const context=item.decision_context||{}; const problem=clean(context.issue_text||item.reason); const recommended=clean(context.recommended_action||item.next_action); const current=clean(context.current_description||context.description); const suggested=clean(context.suggested_description); return `${problem?`<section class="cc-detail-section"><h3>Worum es geht</h3><p>${escapeHtml(problem)}</p></section>`:''}${recommended?`<section class="cc-detail-section cc-detail-section--accent"><h3>Erforderlicher Schritt</h3><p>${escapeHtml(recommended)}</p></section>`:''}${current||suggested?`<div class="cc-copy-compare"><section><span class="cc-kicker">Aktuell</span><p>${escapeHtml(current||'Keine aktuelle Fassung.')}</p></section>${suggested?`<section class="cc-copy-compare__proposal"><span class="cc-kicker">Vorschlag</span><p>${escapeHtml(suggested||'Kein Vorschlag verfügbar.')}</p></section>`:''}</div>`:''}`; }
function actions(item) {
  if(item.case_kind==='startpartner_candidate'){
    const data=startpartnerData(item); const status=String(data?.status||'');
    if(startpartnerAiReviewStatuses.has(status)){
      const hidden=new Set(['edit_qualification','start_prequalification','mark_contact_pending','mark_awaiting_response','start_qualification','mark_decision_ready','accept_pending_terms','waitlist','route_regular','reject']);
      const secondary=asArray(item.secondary_actions).filter(action=>action?.key&&!hidden.has(action.key)).filter((entry,index,list)=>list.findIndex(item=>item.key===entry.key)===index);
      return {primary:null,secondary};
    }
    return {primary:item.primary_action,secondary:asArray(item.secondary_actions)};
  }
  if(item.case_kind==='event_candidate') { const review=contract(item); const ready=Boolean(review?.decision_gate?.ready); return {primary:ready?{key:'approve',label:'Event übernehmen'}:{key:'resolve_review_task',label:review?.summary?.headline||'Offene Punkte klären'},secondary:[...(ready?[{key:'edit_and_approve',label:'Gesamtfassung bearbeiten'}]:[]),{key:'snooze',label:'Gesamten Fall zurückstellen'},{key:'reject',label:'Event ablehnen',destructive:true},{key:'details',label:'Quelldaten'}]}; }
  ...
  const secondary=[...asArray(item.secondary_actions),{key:'details',label:'Details'}].filter((entry,index,list)=>entry?.key&&list.findIndex(item=>item.key===entry.key)===index); return {primary:item.primary_action,secondary};
}
function secondaryActions(available,links,className='') { return `<div class="cc-actions cc-actions--secondary ${className}">${available.secondary.map(action=>`<button class="cc-button ${action.destructive?'cc-button--danger':'cc-button--secondary'}" data-review-action="${escapeHtml(action.key)}">${escapeHtml(action.label)}</button>`).join('')}${links}</div>`; }
function hasCurrentPilot(data){return ['active','paused','closing','ended_without_conversion','terminated','converted'].includes(String(data?.gate4?.phase||''));}
function detail(item,index,total) {
  const available=actions(item); const links=asArray(item.source_links).map(link=>`<a class="cc-button cc-button--secondary" href="${escapeHtml(link.url)}" target="_blank" rel="noopener">${escapeHtml(link.label)}</a>`).join(''); const review=contract(item); const startpartner=item.case_kind==='startpartner_candidate'; const data=startpartnerData(item);
  const startpartnerBody=startpartner
    ? hasCurrentPilot(data)
      ? `${renderGate4Panel(data)}<details class="cc-disclosure cc-startpartner-history"><summary>Aufnahme & Einrichtung</summary>${renderStartpartnerReview(item,{history:true})}</details>`
      : `${renderStartpartnerReview(item)}${renderGate4Panel(data)}`
    : '';
  const body=item.case_kind==='event_candidate'?renderEventReview(review,item):startpartner?startpartnerBody:generic(item); const blockerCount=Number(review?.summary?.open_task_count||0); const secondary=secondaryActions(available,links,'cc-actions--secondary-desktop'); const mobileSecondary=`<details class="cc-mobile-case-options"><summary>Weitere Falloptionen und Quellen</summary>${secondaryActions(available,links,'cc-actions--secondary-mobile')}</details>`; const primary=startpartner?'':`<div class="cc-action-primary ${item.case_kind==='event_candidate'?'cc-action-primary--event-candidate':''}">${available.primary?`<button class="cc-button cc-button--primary cc-button--large" data-review-action="${escapeHtml(available.primary.key)}">${escapeHtml(available.primary.label)}</button>`:`<div class="cc-empty">${escapeHtml(item.waiting_for||'Aktuell keine Aktion erforderlich.')}</div>`}</div>`;
...
export function renderReview() {
  ...
  const queue=items.map((item,index)=>`<button class="cc-queue-item ${index===state.reviewIndex?'is-active':''}" data-review-index="${index}"><span>${escapeHtml(item.title)}</span><small>${escapeHtml(item.case_kind==='startpartner_candidate'?`${caseStatus(item)} · ${item.decision_context?.assigned_to||'nicht zugewiesen'}`:reviewLabels[reviewGroup(item)]||caseStatus(item))}</small></button>`).join(''); const pills=groups.map(([key,label])=>`<button class="cc-filter ${state.reviewGroup===key?'is-active':''}" data-review-group="${key}">${escapeHtml(label)} (${count(key)})</button>`).join(''); const options=groups.map(([key,label])=>`<option value="${key}" ${state.reviewGroup===key?'selected':''}>${escapeHtml(label)} (${count(key)})</option>`).join('');
