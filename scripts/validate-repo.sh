...
validate_routing(){
  ...
  for file in data/control_center_repo_workpacks.json data/control_center_editorial_contract.json data/content_ops_decision_classes.json data/event_identity_contract.json data/event_visual_pool.json tests/fixtures/control_center_editorial_cases.json tests/fixtures/event_identity_cases.json api/sql/000_manifest.json; do python3 -m json.tool "$file" >/dev/null; done
}
validate_php_syntax(){
  find api -type f -name '*.php' -print0 | xargs -0 -n1 php -l >/dev/null
  for file in tests/control_center*.php tests/startpartner_*.php; do php -l "$file" >/dev/null; done
}
validate_preflight(){ php "$PREFLIGHT_TEST"; }
validate_php_tests(){
  for file in tests/control_center*.php; do [ "$file" = "$PREFLIGHT_TEST" ] || php "$file"; done
...
validate_backend(){ echo "== Backend and data contracts =="; validate_routing; validate_php_syntax; validate_preflight; validate_php_tests; validate_startpartner_mysql; }
validate_frontend(){
...
case "$section" in
  backend) validate_backend;; routing) validate_routing;; php-syntax) validate_php_syntax;; preflight) validate_preflight;; php-tests) validate_php_tests;; startpartner-mysql) validate_startpartner_mysql;; frontend) validate_frontend;; repository) validate_repository;; all) validate_backend; validate_frontend; validate_repository;; *) echo "Unknown validation section: $section" >&2; exit 2;;
esac
