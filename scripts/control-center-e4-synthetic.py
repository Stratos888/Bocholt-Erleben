...
class Database:
    ...
    def case_state(self, case_id: str) -> dict[str, Any] | None:
        with self.connection.cursor() as cursor:
            cursor.execute(
                "SELECT id, state, source_system, source_reference, decision_ready FROM control_cases WHERE id=%s",
                (case_id,),
            )
    ...
    def operation_state(self, operation_id: str) -> dict[str, Any] | None:
        with self.connection.cursor() as cursor:
            cursor.execute(
                "SELECT operation_id, case_id, action, status, error_text FROM control_operations WHERE operation_id=%s",
                (operation_id,),
            )
            return cursor.fetchone()
    ...
    def assert_absent(self, run_key: str) -> dict[str, int]:
        with self.connection.cursor() as cursor:
            cursor.execute(
                "SELECT COUNT(*) AS count_value FROM control_cases WHERE source_reference LIKE %s",
                (f"{E4_PREFIX}-{run_key}-%",),
            )
            cases = int(cursor.fetchone()["count_value"])
            cursor.execute(
                "SELECT COUNT(*) AS count_value FROM control_operations WHERE operation_id LIKE %s",
                (f"%{run_key}%",),
            )
...
class GitHubDeploy:
    ...
    def list_runs(self, event: str | None = None) -> list[dict[str, Any]]:
        ...
        payload = self.request(
            f"/repos/{self.repository}/actions/workflows/deploy-strato.yml/runs?{urllib.parse.urlencode(query)}"
        )
    ...
    def dispatch_and_wait(self, label: str, timeout_seconds: int = 1800) -> dict[str, Any]:
        ...
        while time.time() < deadline:
            for run in self.list_runs("workflow_dispatch"):
                run_id = int(run["id"])
                if run_id in before or run.get("head_sha") != self.sha:
                    continue
                selected = run
                break
            if selected and selected.get("status") == "completed":
                if selected.get("conclusion") != "success":
                    raise E4Error(
                        f"{label} deploy failed: run {selected.get('id')} conclusion={selected.get('conclusion')}"
                    )
                return selected
            time.sleep(10)
            if selected:
...
def contains_event_id(payload: Any, event_id: str) -> bool:
    if isinstance(payload, dict):
        if str(payload.get("id", "")) == event_id:
            return True
        return any(contains_event_id(value, event_id) for value in payload.values())
...
def self_test() -> None:
    ...
    assert event["time"] == "19:00–20:00 Uhr"
...
def main() -> int:
    ...
    review_password = require_env("REVIEW_PASSWORD")
    ...
    sheets = SheetClient(sheet_id, service_account_json)
    db = Database()
    http = HttpClient(review_password)
    deploy = GitHubDeploy(github_token, repository, sha)
    case_ids: list[str] = []
    operation_ids: list[str] = []
    primary_error: Exception | None = None

    sheet_ids = sheets.sheet_ids([
        LIVE_EVENTS_TAB,
        LIVE_INBOX_TAB,
        INBOX_TAB,
        EVENTS_TAB,
    ...
    success_event = event_from_payload(success_payload)
    ...
    try:
        ...
        cityart_rows = [
            row_dict(inbox_header, row, offset)
            for offset, row in enumerate(staging_inbox_before[1:], start=2)
        ...
        cityart_event_rows = [
            row for row in staging_events_before[1:]
            if "cityart" in "\n".join(str(cell or "").lower() for cell in row)
        ]
        if cityart_event_rows:
        ...
        baseline = {
            ...
            "cityart_row_number": int(cityart_rows[0]["row_number"]),
            "cityart_status": cityart_rows[0].get("status"),
            "cityart_event_absent": True,
            "synthetic_absent": True,
        }
        evidence["steps"]["baseline"] = baseline

        sheets.append_rows(INBOX_TAB, [
        ...
        staging_inbox_seeded = sheets.values(INBOX_TAB, "A:AB")
        staging_events_seeded = sheets.values(EVENTS_TAB, "A:K")
        success_inbox_rows = find_exact_rows(staging_inbox_seeded, "id_suggestion", success_id)
        resume_inbox_rows = find_exact_rows(staging_inbox_seeded, "id_suggestion", resume_id)
        resume_event_rows = find_exact_rows(staging_events_seeded, "id", resume_id)
        ...
        if resume_event_rows[0].get("title") != resume_title:
            raise E4Error("Seeded resume Event does not match expected content.")
        evidence["steps"]["seed"] = {
        ...
        def create_case(payload: dict[str, str], row_number: str) -> str:
            ...
            source_payload["sheet_tab"] = INBOX_TAB
            source_payload["row_number"] = row_number
        ...
        def run_preflight(case_id: str, allowed_targets: set[str]) -> dict[str, Any]:
            response = http.json_request(
                f"{base_url}/api/control-center/preflight.php",
                method="POST",
                body={"case_id": case_id, "action": "approve", "payload": {}},
            )
            data = assert_api_ok(response, "E4 runtime preflight")["data"]
            return assert_preflight(data, expected_build, allowed_targets)

        success_preflight = run_preflight(success_case, {"absent"})
        resume_preflight = run_preflight(resume_case, {"resolved"})
        evidence["steps"]["preflight"] = {
            "success": success_preflight,
            "resume": resume_preflight,
        }
        ...
        success_result = assert_api_ok(response, "E4 success action")["data"]
        if success_result.get("state") != "done":
            raise E4Error(f"Success case did not reach done: {success_result.get('state')}")
        if success_result.get("writeback", {}).get("event", {}).get("mode") != "appended":
            raise E4Error(f"Success event was not appended: {success_result.get('writeback', {}).get('event')}")
        replay = http.json_request(
            f"{base_url}/api/control-center/action.php",
            method="POST",
            body={
                "case_id": success_case,
        ...
        replay_payload = assert_api_ok(replay, "E4 idempotent replay")
        if replay_payload.get("idempotent_replay") is not True:
        ...
        if blocked.status != 409 or blocked.payload.get("status") != "error":
            raise E4Error(f"Failed operation id was not fail-closed: HTTP {blocked.status} {blocked.payload}")

        resume_operation = f"e4:{run_key}:resume"
        operation_ids.append(resume_operation)
        resume_note = f"E4 synthetic resume {run_key}"
        resumed = http.json_request(
            f"{base_url}/api/control-center/action.php",
            method="POST",
            body={
                "case_id": resume_case,
        ...
        resume_result = assert_api_ok(resumed, "E4 resume action")["data"]
        if resume_result.get("state") != "done":
            raise E4Error(f"Resume case did not reach done: {resume_result.get('state')}")
        if resume_result.get("writeback", {}).get("event", {}).get("mode") != "existing":
            raise E4Error(f"Resume event did not reuse existing row: {resume_result.get('writeback', {}).get('event')}")

        after_action_inbox = sheets.values(INBOX_TAB, "A:AB")
        after_action_events = sheets.values(EVENTS_TAB, "A:K")
        success_events = find_exact_rows(after_action_events, "id", success_id)
        resume_events = find_exact_rows(after_action_events, "id", resume_id)
        success_inbox = find_exact_rows(after_action_inbox, "id_suggestion", success_id)
        resume_inbox = find_exact_rows(after_action_inbox, "id_suggestion", resume_id)
        if len(success_events) != 1 or len(resume_events) != 1:
            raise E4Error("Event duplicate protection failed.")
        if len(success_inbox) != 1 or len(resume_inbox) != 1:
            raise E4Error("Synthetic Inbox identities are not unique after actions.")
        if success_inbox[0].get("status") != "übernommen" or resume_inbox[0].get("status") != "übernommen":
            raise E4Error("Synthetic Inbox statuses were not terminally confirmed.")
        success_case_state = db.case_state(success_case)
        resume_case_state = db.case_state(resume_case)
        if not success_case_state or not resume_case_state:
            raise E4Error("Synthetic local cases are missing before postcondition verification.")
        if success_case_state.get("state") != "done" or resume_case_state.get("state") != "done":
            raise E4Error("Local control cases were not confirmed done.")
        success_operation_state = db.operation_state(success_operation)
        failed_operation_state = db.operation_state(failed_operation)
        resume_operation_state = db.operation_state(resume_operation)
        if not success_operation_state or success_operation_state.get("status") != "completed":
            raise E4Error("Success operation not completed.")
        if not failed_operation_state or failed_operation_state.get("status") != "failed":
            raise E4Error("Seeded failed operation lost fail-closed state.")
        if not resume_operation_state or resume_operation_state.get("status") != "completed":
            raise E4Error("Resume operation not completed.")

        evidence["steps"]["writeback"] = {
            "success": {
                "event_mode": "appended",
                "operation_replay": True,
                "event_rows": len(success_events),
                "inbox_status": success_inbox[0].get("status"),
                "local_state": "done",
            },
            "partial_failure": {
                "seeded_state": "event_confirmed_inbox_review",
                "failed_operation_status": "failed",
                "same_operation_retry_http": blocked.status,
            },
            "resume": {
                "event_mode": "existing",
                "event_rows": len(resume_events),
                "inbox_status": resume_inbox[0].get("status"),
                "local_state": "done",
            },
        }
        ...
        evidence["steps"]["feed"] = {
            ...
            "staging_contains_success": True,
            "staging_contains_resume": True,
        ...
        try:
            ...
            cityart_events_after = [
                row for row in staging_events_after[1:]
                if "cityart" in "\n".join(str(cell or "").lower() for cell in row)
            ]
            if "baseline" in evidence["steps"]:
                baseline = evidence["steps"]["baseline"]
                checks = {
                    "live_inbox_unchanged": canonical_hash(normalize_rows(live_inbox_after)) == baseline["live_inbox_hash"],
                    "live_events_unchanged": canonical_hash(normalize_rows(live_events_after)) == baseline["live_events_hash"],
                    "staging_inbox_non_test_unchanged": canonical_hash(filter_test_rows(staging_inbox_after, run_key)) == baseline["staging_inbox_non_test_hash"],
                    "staging_events_non_test_unchanged": canonical_hash(filter_test_rows(staging_events_after, run_key)) == baseline["staging_events_non_test_hash"],
                }
            else:
                checks = {}
            checks.update({
                "synthetic_inbox_absent": not find_exact_rows(staging_inbox_after, "id_suggestion", success_id)
                    and not find_exact_rows(staging_inbox_after, "id_suggestion", resume_id),
                "synthetic_events_absent": not find_exact_rows(staging_events_after, "id", success_id)
            ...
            evidence["cleanup"]["checks"] = checks
            evidence["cleanup"]["database_remaining"] = db_absent
            failed_checks = [name for name, passed in checks.items() if not passed]
            if failed_checks:
                cleanup_errors.append("cleanup_assertions: " + ", ".join(failed_checks))
        except Exception as error:
