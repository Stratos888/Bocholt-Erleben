...
function be_cc_provider_event_quality_metrics(PDO $pdo): array
{
    ...
    foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
        ...
        $source = trim((string)($row['event_url'] ?? $row['ticket_url'] ?? ''));
        if ($source === '') $source = trim((string)($row['ticket_url'] ?? ''));
...
{
    $items = be_cc_activity_items(true);
    $total = $complete = $missingDescription = $missingSource = 0;
    foreach ($items as $item) {
        ...
        if ($description === '') $missingDescription++;
        if ($source === '') $missingSource++;
        if ($description !== '' && $source !== '') $complete++;
    }
    return compact('total','complete','missingDescription','missingSource');
}
...
function be_cc_sum_quality(array $left, array $right): array
{
    return [
        ...
        'complete' => (int)($left['complete'] ?? 0) + (int)($right['complete'] ?? 0),
        'missingDescription' => (int)($left['missingDescription'] ?? 0) + (int)($right['missingDescription'] ?? 0),
        'missingSource' => (int)($left['missingSource'] ?? 0) + (int)($right['missingSource'] ?? 0),
        'missingDate' => (int)($left['missingDate'] ?? 0) + (int)($right['missingDate'] ?? 0),
    ];
}
...
try {
    ...
    $activityCoverage = (int)$activityQuality['total'] > 0 ? round(((int)$activityQuality['complete'] / (int)$activityQuality['total']) * 100, 1) : 0.0;
    ...
    be_json_response(200, ['status'=>'ok','data'=>[
        ...
        'content_quality'=>[
            'events_total'=>$total,
            'sheet_events_total'=>(int)$sheetQuality['total'],
            'provider_events_total'=>(int)$providerQuality['total'],
            'complete_events'=>$complete,
            'coverage_percent'=>$coverage,
            'missing_description'=>(int)$eventQuality['missingDescription'],
            'missing_source'=>(int)$eventQuality['missingSource'],
            'missing_date'=>(int)$eventQuality['missingDate'],
            'activities_total'=>(int)$activityQuality['total'],
            'complete_activities'=>(int)$activityQuality['complete'],
            'activity_coverage_percent'=>$activityCoverage,
            'activity_missing_description'=>(int)$activityQuality['missingDescription'],
            'activity_missing_source'=>(int)$activityQuality['missingSource'],
