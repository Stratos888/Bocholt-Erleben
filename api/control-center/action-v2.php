...
function be_cc_v2_recheck_source_case(array $case): array
{
...
try {
    ...
    if ($action === 'recheck') {
        $verification = be_cc_v2_recheck_source_case($case);
    ...
    if ($sourceSystem === 'inbox_feed') {
        $runtimeContract = be_cc_assert_inbox_case_environment(be_cc_editorial_payload($case));
        $payload['runtime_environment'] = $runtimeContract;
    }
    ...
    if (is_array($runtimeContract)) $writebackMeta['runtime_environment'] = $runtimeContract;
