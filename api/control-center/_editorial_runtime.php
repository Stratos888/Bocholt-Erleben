...
{
    $aliases = ['end_date'=>'endDate','category'=>'kategorie_suggestion','source_url'=>'source_url','ticket_url'=>'ticket_url','visual_motif'=>'visual_motif'];
    foreach ($updates as $key=>$value) $source[$aliases[$key] ?? $key] = is_array($value) ? $value : trim((string)$value);
...
function be_cc_operation_reserve_persistent(PDO $pdo, string $operationId, string $caseId, string $action, array $payload): array
{
    ...
    if (is_array($existing)) {
        if ((string)$existing['case_id'] !== $caseId || (string)$existing['action'] !== $action || (string)$existing['payload_hash'] !== $hash) throw new DomainException('operation_id wurde bereits für einen anderen Vorgang verwendet.');
...
function be_cc_operation_finish(PDO $pdo, string $operationId, string $status, ?array $result = null, string $error = ''): void
{
    ...
    $stmt->execute(['status'=>$status,'result'=>$result===null?null:json_encode($result,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_THROW_ON_ERROR),'error'=>$error!==''?$error:null,'done'=>in_array($status,['completed','failed'],true)?1:0,'id'=>$operationId]);
}
...
function be_cc_update_sheet_row_canonical(string $tab, int $headerRow, int $rowNumber, array $updates): array
{
    ...
    $header=array_map(static fn($value):string=>strtolower(trim((string)$value)),(array)($response['values'][0]??[]));
...
function be_cc_writeback_audit_stable(array $case,string $action,array $payload,array $decision):array
{
    ...
    if(($resolution['status']??'')==='resolved'){
        ...
        $updates=match($action){
            'approve'=>$decision['event_updates']?['status'=>'corrected','action_state'=>'event_sheet_corrected','verification_status'=>'corrected']:['status'=>'verified','action_state'=>'verified_until_next_review','verification_status'=>'confirmed'],
...
function be_cc_record_editorial_feedback(PDO $pdo,array $case,array $decision,array $payload):void
{
    ...
    $source=be_cc_editorial_payload($case);
    $eventId=trim((string)($source['content_id']??$case['object_id']??''));
    ...
    if($eventId===''||$final==='')return;
    $observation=be_cc_editorial_feedback_observation($eventId,$issueCode,$before,$suggested,$final,[
        'city'=>$source['city']??'','location'=>$source['location']??'',
        'source_fingerprint'=>$source['source_fingerprint']??'','content_fingerprint'=>$source['content_fingerprint']??'',
        'prompt_rule_version'=>$payload['rule_version']??'control-center-editorial-v1',
    ]);
    ...
    $stmt->execute([
        'id'=>be_cc_uuid(),'case_id'=>$case['id'],'object_id'=>$eventId,'issue_code'=>$issueCode,'before_text'=>$before,'suggested_text'=>$suggested,'final_text'=>$final,
        ...
        'rule_version'=>(string)($observation['prompt_rule_version']??''),
    ]);
}
