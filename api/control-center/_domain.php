...
require_once __DIR__ . '/_workflow_contracts.php';
...
function be_cc_require_schema(): void
{
    ...
    if (!$exists) {
        be_json_response(503, ['status'=>'error','code'=>'CONTROL_CENTER_SCHEMA_MISSING','message'=>'Die Steuerzentrale ist noch nicht initialisiert.']);
    }
}
...
function be_cc_case_from_row(array $row): array
{
    ...
    $case = [
        'id'=>(string)$row['id'],'type'=>(string)$row['case_type'],'state'=>$state,'priority'=>(string)$row['priority'],
        'title'=>(string)$row['title'],'reason'=>(string)($row['reason'] ?? ''),'next_action'=>(string)($row['next_action'] ?? ''),
        'object'=>['type'=>(string)($row['object_type'] ?? ''),'id'=>(string)($row['object_id'] ?? ''),'title'=>(string)($row['object_title'] ?? '')],
        'source'=>['system'=>(string)$row['source_system'],'reference'=>(string)$row['source_reference']],
        'due_at'=>$row['due_at'] ?? null,'snoozed_until'=>$row['snoozed_until'] ?? null,'blocked_reason'=>(string)($row['blocked_reason'] ?? ''),
        'decision_ready'=>(bool)$row['decision_ready'],'overdue'=>$overdue,'bucket'=>$bucket,'created_at'=>$row['created_at'] ?? null,
        'updated_at'=>$row['updated_at'] ?? null,'completed_at'=>$row['completed_at'] ?? null,
    ];
...
function be_cc_list_cases(array $filters = []): array
{
    ...
    $where = []; $params = [];
    if (!empty($filters['type'])) { $where[] = 'case_type = :type'; $params['type'] = be_cc_validate_enum((string)$filters['type'], BE_CC_TYPES, 'type'); }
    if (!empty($filters['state'])) { $where[] = 'state = :state'; $params['state'] = be_cc_validate_enum((string)$filters['state'], BE_CC_STATES, 'state'); }
    ...
    $stmt = be_db()->prepare($sql); $stmt->execute($params);
...
function be_cc_record_event(PDO $pdo, string $caseId, string $action, ?string $from, ?string $to, array $payload = [], string $actor = 'operator'): void
{
    ...
    $stmt->execute([
        'case_id'=>$caseId,'action'=>$action,'from_state'=>$from,'to_state'=>$to,'actor'=>$actor,
...
function be_cc_apply_action(string $caseId, string $action, array $payload): array
{
    ...
    try {
        ...
        $to = be_cc_transition_target($action, $from);
        $fields = ['state = :state','updated_at = NOW()'];
        $params = ['state'=>$to,'id'=>$caseId];
        if ($action === 'convert_to_task') { $fields[] = "case_type = 'task'"; $fields[] = 'decision_ready = 0'; }
        if (in_array($to, ['done','rejected'], true)) $fields[] = 'completed_at = NOW()'; else $fields[] = 'completed_at = NULL';
        if ($action === 'snooze') {
        ...
        if (in_array($action, ['wait','block'], true)) {
            ...
            $fields[] = 'blocked_reason = :blocked_reason'; $params['blocked_reason'] = $reason;
        } elseif (!in_array($to, ['waiting','blocked'], true)) $fields[] = 'blocked_reason = NULL';
        ...
        $pdo->commit();
        $stmt = $pdo->prepare('SELECT * FROM control_cases WHERE id = :id'); $stmt->execute(['id'=>$caseId]);
