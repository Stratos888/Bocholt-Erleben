...
function be_cc_upsert_source_case(array $case): string
{
    ...
    if ($sourceSystem === '' || $sourceReference === '') {
        throw new InvalidArgumentException('Source identity is required.');
    }
    ...
    if ($existing) {
        ...
        if (in_array((string)$existing['state'], ['done', 'rejected'], true)) {
            return $id;
        }
        $stmt = $pdo->prepare(
...
function be_cc_sync_inbox_feed(): array
{
    ...
    foreach ($items as $index => $item) {
        ...
        be_cc_upsert_source_case([
            ...
            'next_action' => 'Übernehmen, ablehnen oder als konkrete Aufgabe weiterführen.',
...
function be_cc_sync_content_audit(): array
{
    ...
    for ($i = 3; $i < count($values); $i++) {
        ...
        be_cc_upsert_source_case([
            ...
            'object_id' => $contentId,
            'object_title' => $cell($row, 'title'),
            'source_system' => 'content_audit',
            'source_reference' => $reference,
            'source_payload' => array_combine($header, array_pad($row, count($header), '')) ?: [],
            'decision_ready' => true,
        ]);
        $count++;
    }
